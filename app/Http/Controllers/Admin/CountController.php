...
class CountController extends AdminController{
    ...
    public function GetCount(){
        ...
        return view('admin.counts' , ['data' => $data]);
    }

    // post update counts
    public function PostCount(Request $request){
        $count = Count::find(1);
        $count->user = $request->beneficiaries_count;
        $count->service = $request->services_count;
        $count->family = $request->family;
        $count->initiative = $request->initiative;
        $count->save();
        return back();
    }
