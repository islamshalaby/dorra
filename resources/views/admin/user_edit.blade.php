...
@section('title' , 'Admin Panel User Edit')
...
<form method="post" action="" >
    ...
    {{-- <div class="form-group mb-4">
        ...
        <select name="gender" required class="form-control" id="gender">
            ...
            <option {{$data['user']['gender'] == 0 ? 'selected' : '' }} value="0" >{{ __('messages.female') }}</option>
        </select>
    </div> --}}
    {{-- <div class="form-group mb-4">
        <label for="DateOfBirth">{{ __('messages.date_of_birth') }}</label>
        <input name="date_of_birth" required type="date" class="form-control" id="DateOfBirth" placeholder="" value="{{$data['user']['date_of_birth']}}" >
    </div>       --}}
